...
**No auto-renewal**. Daily cron `/api/cron/pro-renewal-reminders` emails prepaid users at ~7d and ~1d before `pro_until` with a renewal link (`/zh/pricing?plan=pro`).  
Webhook: `payment_intent.succeeded` → `grantProPrepaid` (sets `pro_billing=prepaid`). Default CNY cycle in UI: **quarterly**.
