...
export async function checkFreeQuota(input: QuotaCheckInput): Promise<QuotaCheckResult> {
  ...
  if (usePostgres()) {
    ...
    if (!rows[0]) {
      return { allowed: true, inTrialPeriod: true, remaining: -1 };
    }

    const row = rows[0];
    const trialStart = row.trial_start?.toISOString() ?? null;
    if (inTrial(trialStart)) {
      return { allowed: true, inTrialPeriod: true, remaining: -1 };
    }
...
export async function recordFreeScan(input: QuotaCheckInput): Promise<void> {
  ...
  if (usePostgres()) {
    ...
    if (!rows[0]) {
      await db`
        ...
        VALUES (${key}, ${now}, ${monthKey}, 0)`;
      return;
    }

    const row = rows[0];
    const trialStart = row.trial_start?.toISOString() ?? now;
    if (inTrial(trialStart)) {
      if (!row.trial_start) {
        await db`UPDATE scan_quota SET trial_start = ${now}, updated_at = NOW() WHERE id = ${key}`;
      }
      return;
    }
  ...
  if (!row) {
    row = { id: key, trialStart: now, monthKey, scanCount: 0 };
