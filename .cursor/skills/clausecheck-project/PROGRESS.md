...
- 每日 Cron `0 1 * * *` UTC → `/api/cron/pro-renewal-reminders`（7 天 + 1 天窗口，audit 幂等）。
- 邮件含续费链接 `/zh/pricing?plan=pro`；无邮箱的手机号账号跳过。
...
- `payment_intent.succeeded` → `grantProPrepaid`：写 `pro_until`（月 30 / 年 365 天，可叠加）+ 同步 Pro 配额；audit 幂等。
- UI：CNY CTA「立即购买（预付）」+ 结账预付说明；去掉结账窗内误导性「联系我们」CTA。
...
- 报告更新：`docs/I18N_COMPLIANCE_DIFF_REPORT.md`；EXPERT_BRIEF 同步。外部剩余：法务对照供应商 DPA 签字。
- **微信 #3：** `isWechatPayUiEnabled` 改为开关 ∧ 收银已配；新增 `docs/WECHAT_PAY_ENABLEMENT.md`。生产仍缺商户 `WECHAT_PAY_QR_BASE`，**未**开 `WECHAT_PAY_ENABLED`。
